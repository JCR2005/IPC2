...
export class guardSuscriptor implements CanActivate {
  ...
  canActivate(): boolean {
    ...
    if (token) {
      try {
        ...
        if (payload && payload.tipoCuenta === 'Suscriptor') {
          return true;
        }
      } catch (error) {
    ...
    this.router.navigate(['/login']);
    alert('No tienes permiso para acceder a esta página. Inicia sesión para poder ingresar.');
    return false;
  }
