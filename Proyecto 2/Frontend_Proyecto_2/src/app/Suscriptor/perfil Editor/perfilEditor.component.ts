...
@Component({
  ...
  templateUrl: './perfilEditor.component.html',
  styleUrls: ['./perfilEditor.component.css']
})
export class PerfilEditorComponent implements OnInit {
  ...
  constructor(
    ...
    private token: token,private router: Router, private route: ActivatedRoute,
  ) {}

  ngOnInit(): void {
    this.route.queryParams.subscribe((params) => {
      this.mensajeRecibido = params['Autor'] || 'No se recibió ningún mensaje';
    });
    this.usuario=this.mensajeRecibido;
    this.cargarPerfil();
  }
