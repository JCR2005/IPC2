...
export class AnuncianteGuardia implements CanActivate {
  ...
  canActivate(): boolean {
    ...
    if (token) {
      try {
        ...
        if (payload && payload.tipoCuenta === 'Anunciante') {
          return true;
        }
      } catch (error) {
    ...
    this.router.navigate(['/login']);
    alert('No tienes permiso para acceder a esta página. Inicia sesión como Anunciante.');
    return false;
  }
